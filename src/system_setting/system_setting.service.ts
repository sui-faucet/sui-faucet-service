...
export class SystemSettingService {
  constructor(
    ...
    private systemSettingModel: Model<SystemSetting>,
  ) {}

  async create(
    createSystemSettingDto: CreateSystemSettingDto,
  ): Promise<SystemSetting> {
    const existingSystemSetting = await this.systemSettingModel
      .findOne()
      .exec();
    if (existingSystemSetting) {
      throw new BadRequestException('System setting already exists');
    }
    const createdSystemSetting = new this.systemSettingModel(
